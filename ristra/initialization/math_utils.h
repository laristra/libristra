...
// user includes
#include "ristra/initialization/detail/math_utils_impl.h"

#include "ristra/initialization/detail/template_helpers.h"
...
constexpr std::enable_if_t<(sizeof...(Types) > 1) &&
    ::ristra::are_type_t<T, Types...>::value,
  std::decay_t<T> >
...
constexpr std::enable_if_t< ::ristra::are_type_t<T, U>::value, std::decay_t<T> >
average(T && t, U && u)
{
...
//! \brief average operator.
//! \remark all arguments must be of the same type
//! \remark this function gives an error because not all values are the same
//! type
template <class T, class... Types>
constexpr std::enable_if_t<!::ristra::are_type_t<T, Types...>::value,
  std::decay_t<T> >
average(T && t, Types &&... args)
{
  static_assert(::ristra::are_type_t<T, Types...>::value,
...
template <typename InputIt,
  typename = typename std::enable_if_t< ::ristra::is_iterator_v<InputIt> > >
...
template <typename T, std::size_t... N,
  template <typename, std::size_t...> class A,
  std::size_t Len = detail::multiply(N...)>
T dot_product(const A<T, N...> & a, const A<T, N...> & b)
{
  T dot = 0;
  for (detail::select_counter_t<Len> i = 0; i < Len; ++i)
    dot += a[i] * b[i];
  return dot;
}
